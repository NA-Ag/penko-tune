...
import { Music, Play, Trash2, MoreVertical, Plus, X, Image, Cloud, FileAudio } from 'lucide-react';
            ...
            return (
              ...
              >
                ...
                <td className="py-3 hidden sm:table-cell text-zinc-500 uppercase text-xs">
                  <div className="flex items-center gap-2">
                    {track.type === 'stream' ? (
                      <><Cloud size={14} className="text-cyan-500" /> STREAM</>
                    ) : (
                      <><FileAudio size={14} /> {track.file?.type.split('/')[1] || 'AUDIO'}</>
                    )}
                  </div>
                </td>
