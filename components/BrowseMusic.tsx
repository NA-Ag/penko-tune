...
import { Search, Music, User, Globe, X, Play, ExternalLink, Copy, Heart, Plus } from 'lucide-react';
...
export const BrowseMusic: React.FC<BrowseMusicProps> = ({ onClose, onPlayTrack, onAddToLibrary, addToast }) => {
  ...
  const convertToTrack = (track: DecentralizedTrack): Track => {
    ...
    return {
      ...
      type: 'stream',
      // Preserve P2P metadata for the resolver
      ...track 
    };
  };

  const handlePlayTrack = (track: DecentralizedTrack) => {
    const playableTrack = convertToTrack(track);
    onPlayTrack(playableTrack);
  };

  const handleAddToLibrary = (track: DecentralizedTrack) => {
    const playableTrack = convertToTrack(track);
    onAddToLibrary(playableTrack);
  };
